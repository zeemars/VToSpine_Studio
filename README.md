...
- 预览循环播放
- 浮动预览窗口（可拖动、最小化）
- 设置保存功能（可保存和加载抠图参数、画布设置等）
- 间隔选择帧功能（可设置间隔几帧选中一张图片）
- 文件类型验证（只允许上传视频文件）
...
- 系统会自动提取帧并应用抠图
- 可手动调整抠图参数（背景色、阈值、平滑度、边缘收缩）
- 可调整画布大小和位置
- 可启用像素化效果并设置像素大小
- 可启用空白裁剪并设置裁剪模式和边距
- 可保存当前设置为配置文件，方便后续使用
- 可使用间隔选择功能，设置间隔几帧选中一张图片
- 可选择/取消选择要包含的帧

### 3. 导出设置
- 调整导出参数（帧率、前缀）
- 调整精灵表设置（布局、列数、帧尺寸、缩放算法、间距）
- 选择是否导出为单张图片
- 选择导出格式：
  - **Download Spine ZIP**：导出完整的 Spine 项目文件
  - **Export PNG**：导出精灵表 PNG 文件（或单张图片）
...
├── utils/
│   ├── imageProcessing.ts  # 图像处理（帧提取、抠图、像素化）
